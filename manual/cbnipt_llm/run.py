...
#
    ...
    cmd = f'''/storage/home/jhkim/Apps/Python-3.11.13/python syndrome_annotator.py --syndrome "{disease}" \
    ...
    '''
    cmd = f'''
    /storage/home/jhkim/Apps/Python-3.11.13/python -m nipt_pipeline.syndrome_discovery \
        --syndrome "{disease}" --email you@example.com \
      --clingen-gene {resources_path}/ClinGen_gene_curation_list_GRCh38.tsv \
      --hpo {resources_path}/phenotype_to_genes.txt \
      --uniprot-dat {resources_path}/uniprot_sprot.dat.gz \
        --output-dir ./output/{disease.replace(" ","_")}
    '''
    print(cmd)
    os.system(cmd)
