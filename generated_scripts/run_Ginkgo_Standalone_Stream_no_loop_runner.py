...
CMD_LINE = 'mkdir -p [WorkDir] && ln -Tsf [NormalDir]/*.bed.gz [WorkDir]/ && ln -Tsf [BamToBedDir]/[SeqID].bed.gz [WorkDir]/ && find [WorkDir] -maxdepth 1 -name "*.gz" | xargs -I {] bash -c \'N=$(basename "{]" | sed "s/\\.bed.*//"); zcat "{]" | awk "{if(\\$1!~/^chr/) print \\"chr\\"\\$0; else print \\$0]" | gzip > "{]"_t.gz && [BinUnsorted] [BinFile] [BinCount] <(zcat "{]"_t.gz) $N "{]"_mapped\' && find [WorkDir] -maxdepth 1 -name "*.bed" ! -name "*.gz" | xargs -I {] bash -c \'N=$(basename "{]" | sed "s/\\.bed.*//"); awk "{if(\\$1!~/^chr/) print \\"chr\\"\\$0; else print \\$0]" "{]" > "{]"_t && [BinUnsorted] [BinFile] [BinCount] "{]"_t $N "{]"_mapped\' && paste [WorkDir]/*_mapped > [WorkDir]/data && cd [GinkgoHomeDir] && [ProcessR] [GinkgoHomeDir]/genomes/[Genome]/original [WorkDir] [R_Args_Base] && [ReclustR] [GinkgoHomeDir]/genomes/[Genome]/original [WorkDir] [R_Args_Reclust] && [CNVCaller] [WorkDir]/SegCopy [WorkDir]/CNV1 [WorkDir]/CNV2 && mkdir -p [FinalDir] && mv [WorkDir]/* [FinalDir]/ && rm -rf [WorkDir]'
...
def main():
    ...
    for k in ['BamToBedDir', 'BinCount', 'BinFile', 'BinMeth', 'BinSize', 'BinUnsorted', 'CNVCaller', 'FinalDir', 'Genome', 'GinkgoHomeDir', 'NormalDir', 'NormalSamples', 'ProcessR', 'R_Args_Base', 'R_Args_Reclust', 'ReadLength', 'ReclustR', 'ResultBaseDir', 'SeqID', 'WorkDir']:
