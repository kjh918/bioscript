...
usage() {
    ...
    echo "  --qcResDir        Output directory for alignment summary metrics"
...
cmd="${singularity_bin} exec -B ${bind} ${sif} ${java_bin} -XX:ParallelGCThreads=${Threads} -Xmx${xmx_mb}m -jar ${gatk_jar} CollectAlignmentSummaryMetrics --INPUT ${BamDir}/${SeqID}.${InputSuffix}.bam --OUTPUT ${alignment_summary_metrics_txt} --REFERENCE_SEQUENCE ${ReferenceFasta}"
