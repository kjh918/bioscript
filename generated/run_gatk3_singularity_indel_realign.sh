#!/usr/bin/env bash
set -euo pipefail

# Runner: gatk3 3.8 (singularity_indel_realign)

# defaults (override by CLI)
SeqID=''
BamDir=''
qcResDir=''
ReferenceFasta=''
KnownIndel1=''
KnownIndel2=''
bind=/storage,/data
sif=/storage/images/gatk-3.8-1.sif
Threads=8
xmx_mb=16384
target_intervals='[qcResDir]/[SeqID].realign.target.intervals'
realigned_bam='[BamDir]/[SeqID].sorted.dedup.realign.bam'

CWD="."
PRINT_CMD=0
PRINT_OUTPUTS=0
EMIT_OUTPUTS=""

OUTPUT_KEYS=(target_intervals realigned_bam)

usage() {
  echo "Usage: $0 --<Key> <val> ... [--cwd <dir>] [--print-cmd] [--print-outputs] [--emit-outputs <file>]" >&2
}

while [[ $# -gt 0 ]]; do
  case "$1" in
    --SeqID) SeqID="$2"; shift 2;;
    --BamDir) BamDir="$2"; shift 2;;
    --qcResDir) qcResDir="$2"; shift 2;;
    --ReferenceFasta) ReferenceFasta="$2"; shift 2;;
    --KnownIndel1) KnownIndel1="$2"; shift 2;;
    --KnownIndel2) KnownIndel2="$2"; shift 2;;
    --bind) bind="$2"; shift 2;;
    --sif) sif="$2"; shift 2;;
    --Threads) Threads="$2"; shift 2;;
    --xmx_mb) xmx_mb="$2"; shift 2;;
    --target_intervals) target_intervals="$2"; shift 2;;
    --realigned_bam) realigned_bam="$2"; shift 2;;
    --cwd) CWD="$2"; shift 2;;
    --print-cmd) PRINT_CMD=1; shift;;
    --print-outputs) PRINT_OUTPUTS=1; shift;;
    --emit-outputs) EMIT_OUTPUTS="$2"; shift 2;;
    -h|--help) usage; exit 0;;
    *) echo "Unknown arg: $1" >&2; usage; exit 2;;
  esac
done

# required inputs
[[ -n "${SeqID}" ]] || { echo "Missing required --SeqID" >&2; exit 2; }
[[ -n "${BamDir}" ]] || { echo "Missing required --BamDir" >&2; exit 2; }
[[ -n "${qcResDir}" ]] || { echo "Missing required --qcResDir" >&2; exit 2; }
[[ -n "${ReferenceFasta}" ]] || { echo "Missing required --ReferenceFasta" >&2; exit 2; }
[[ -n "${KnownIndel1}" ]] || { echo "Missing required --KnownIndel1" >&2; exit 2; }
[[ -n "${KnownIndel2}" ]] || { echo "Missing required --KnownIndel2" >&2; exit 2; }

render() {
  local s="$1"
  s="${s//[SeqID]/${SeqID}}"
  s="${s//[BamDir]/${BamDir}}"
  s="${s//[qcResDir]/${qcResDir}}"
  s="${s//[ReferenceFasta]/${ReferenceFasta}}"
  s="${s//[KnownIndel1]/${KnownIndel1}}"
  s="${s//[KnownIndel2]/${KnownIndel2}}"
  s="${s//[bind]/${bind}}"
  s="${s//[sif]/${sif}}"
  s="${s//[Threads]/${Threads}}"
  s="${s//[xmx_mb]/${xmx_mb}}"
  s="${s//[target_intervals]/${target_intervals}}"
  s="${s//[realigned_bam]/${realigned_bam}}"
  s="$(echo "$s" | tr -s ' ' | sed 's/^ *//; s/ *$//')"
  echo "$s"
}

emit_outputs() {
  local out=""
  for k in "${OUTPUT_KEYS[@]}"; do
    out+="${k}\t${!k}\n"
  done
  if [[ "$PRINT_OUTPUTS" -eq 1 ]]; then
    printf "%b" "$out"
  fi
  if [[ -n "$EMIT_OUTPUTS" ]]; then
    printf "%b" "$out" > "$EMIT_OUTPUTS"
  fi
}

# finalize outputs
if [[ -z "${target_intervals}" ]]; then
  __tmp='[qcResDir]/[SeqID].realign.target.intervals'
  target_intervals="$(render "$__tmp")"
fi
if [[ -z "${realigned_bam}" ]]; then
  __tmp='[BamDir]/[SeqID].sorted.dedup.realign.bam'
  realigned_bam="$(render "$__tmp")"
fi

CMD_LINE='singularity exec -B [bind] [sif] java -Xmx[xmx_mb]m -jar /usr/GenomeAnalysisTK.jar -T RealignerTargetCreator -R [ReferenceFasta] -I [BamDir]/[SeqID].sorted.dedup.bam -o [qcResDir]/[SeqID].realign.target.intervals -known [KnownIndel1] -known [KnownIndel2] -nt [Threads] && singularity exec -B [bind] [sif] java -Xmx[xmx_mb]m -jar /usr/GenomeAnalysisTK.jar -T IndelRealigner -R [ReferenceFasta] -targetIntervals [qcResDir]/[SeqID].realign.target.intervals -known [KnownIndel1] -known [KnownIndel2] -I [BamDir]/[SeqID].sorted.dedup.bam -o [BamDir]/[SeqID].sorted.dedup.realign.bam'
CMD="$(render "$CMD_LINE")"

if [[ "$PRINT_CMD" -eq 1 ]]; then
  echo "$CMD"
  exit 0
fi

mkdir -p "$CWD"
bash -lc "cd \"$CWD\" && $CMD"

emit_outputs
