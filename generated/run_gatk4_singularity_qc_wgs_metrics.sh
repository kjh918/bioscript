...
# Runner: gatk4 4.4.0.0 (singularity_qc_wgs_metrics)
...
java_bin=java
wgs_metrics_txt='[qcResDir]/[SeqID].collect.wgs.metrics.txt'
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --wgs_metrics_txt) wgs_metrics_txt="$2"; shift 2;;
    --cwd) CWD="$2"; shift 2;;
...
render() {
  ...
  s="${s//[wgs_metrics_txt]/${wgs_metrics_txt}}"
...
# finalize outputs
if [[ -z "${wgs_metrics_txt}" ]]; then
  __tmp='[qcResDir]/[SeqID].collect.wgs.metrics.txt'
  wgs_metrics_txt="$(render "$__tmp")"
fi
