...
def main():
    ...
    parser.add_argument('--alignment_summary_metrics_txt', required=False, default='[qcResDir]/[SeqID].[InputSuffix].alignment_summary_metrics.txt', help='Text file containing summarized alignment metrics (Default: [qcResDir]/[SeqID].[InputSuffix].alignment_summary_metrics.txt)')
    ...
    alignment_summary_metrics_txt = args.alignment_summary_metrics_txt
    ...
    # --- [Output Paths] ---
    if not alignment_summary_metrics_txt:
    ...
    if alignment_summary_metrics_txt:
        _tgt = os.path.dirname(alignment_summary_metrics_txt) if os.path.splitext(alignment_summary_metrics_txt)[1] else alignment_summary_metrics_txt
        if _tgt: os.makedirs(_tgt, exist_ok=True)
    if BamDir:
