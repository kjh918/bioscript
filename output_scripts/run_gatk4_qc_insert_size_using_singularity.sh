...
usage() {
    ...
    echo "  --qcResDir        Output directory for QC metrics and PDF histogram"
    ...
    echo "  --gatk_jar        Path to GATK local jar file inside container (Default: /gatk/gatk-package-4.4.0.0-local.jar)"
...
cmd="${singularity_bin} exec -B ${bind} ${sif} ${java_bin} -XX:ParallelGCThreads=${Threads} -Xmx${xmx_mb}m -jar ${gatk_jar} CollectInsertSizeMetrics --INPUT ${BamDir}/${SeqID}.${InputSuffix}.bam --OUTPUT ${insert_size_metrics_txt} --Histogram_FILE ${insert_size_hist_pdf} --REFERENCE_SEQUENCE ${ReferenceFasta}"
