...
usage() {
    ...
    echo "  --KnownSnp1       Path to known SNPs VCF (e.g., dbSNP)"
    echo "  --KnownSnp2       Path to known SNPs VCF (e.g., dbSNP)"
...
if [[ -z "${KnownSnp1:-}" ]]; then echo "Error: --KnownSnp1 is required"; usage; fi
if [[ -z "${KnownSnp2:-}" ]]; then echo "Error: --KnownSnp2 is required"; usage; fi
