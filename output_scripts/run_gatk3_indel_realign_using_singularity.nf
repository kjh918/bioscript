...
process GATK3 {
    ...
    input:
    tuple val(sample_id), val(SeqID), path(BamDir), val(qcResDir), val(ReferenceFasta), val(TargetBed), val(KnownIndel1), val(KnownIndel2)
    ...
    def InputSuffix = params.InputSuffix ?: "merged.dup.marked"
    def OutputSuffix = params.OutputSuffix ?: "merged.dup.marked.realign"
