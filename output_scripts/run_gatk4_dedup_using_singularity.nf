...
process GATK4 {
    ...
    def optical_duplicate_pixel_distance = params.optical_duplicate_pixel_distance ?: "2500"
    def other_md_args = params.other_md_args ?: ""
    """
    ${singularity_bin} exec -B ${bind} ${sif} ${gatk_bin} MarkDuplicates --java-options '-XX:ParallelGCThreads=${Threads} -Xmx${xmx_mb}m' --INPUT ${BamDir}/${SeqID}.${InputSuffix}.bam --OUTPUT ${dedup_bam} --METRICS_FILE ${metrics_txt} --CREATE_INDEX ${create_index} --REMOVE_SEQUENCING_DUPLICATES ${remove_seq_dups} --OPTICAL_DUPLICATE_PIXEL_DISTANCE ${optical_duplicate_pixel_distance} ${other_md_args}
    """
}