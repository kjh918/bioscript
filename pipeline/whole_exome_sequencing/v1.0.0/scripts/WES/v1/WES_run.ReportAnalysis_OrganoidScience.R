...
#---/ FUNCTIONS /----------------------------------------------------------------------------------#
    ...
    source("/storage/home/jhkim/Projects/NGS/WES/v1/WES_Fun.ReportModules.R")
    source("/storage/home/jhkim/Projects/NGS/WES/v1/NGS_Fun.ReportModules_v2.R")
...
#---/ ANALYSIS : PRESET GENES VARIANTS /------------------------------------------------------------
    ...
    if(  nrow(sinfo_preset_vars_tonly) > 0 )
    {
        ...
        print(PresetGeneVarTableListTonly)
        # export result for report
        PresetGeneVarTableListTonlyReport <- lapply(PresetGeneVarTableListTonly, function(pgv)
        {
            if( nrow(pgv) > 0 )
            {
                res = pgv %>% dplyr::select(c("sample_group","sample_name","GENE_SYMBOL", "varDNA","HGVSp_Short","TUMOR_DEPTH","TUMOR_ALT_COUNT","VAF")) 
            }else{
                res = data.frame()
            }
            return(res)
        ...
        for( k in 1:nrow(AnalysisTonly_vaf) )
        {
            ...
            raw_variants_list <- lapply( indexing(union(analysis_list$s1, analysis_list$s2)), function(idx)
            {
                read.delim(sprintf("%s/%s/%s/vcf/%s.mutect2.bias.filtered.vep.annotated.maf", BaseDir, SeqFolderID, idx, idx)) %>%
                    mutate(VAF = round(TUMOR_ALT_COUNT/TUMOR_DEPTH, 3) ) %>% dplyr::select(c("VKEY","seq_id","VAF"))
            })
