...
#---| FUNCTIONS |----------------------------------------------------------------------------------#
    ...
    PRESET_GENE_VARIANTS_TABLE2 <- function( PF_VARS, CANCER_TISSUE, GENE_LIST, SAMPLE_INFO, PRESET_NAME, VARIANT_CALL_METHOD )
    {
        ...
        presetGeneVarsTable <- PF_VARS %>% 
            ...
            mutate( SampleName = SAMPLE_INFO[match(seq_id, SAMPLE_INFO$seq_id), c("sample_name")], varDNA=paste(REFSEQ_TRANSCRIPT_ID,HGVSc, sep=":")) %>%
            arrange( seq_id, Chromosome, Start_Position) %>% 
            select(c("SampleName","GENE_SYMBOL", "varDNA","HGVSp_Short","TUMOR_DEPTH","TUMOR_ALT_COUNT","VAF")) %>%
            dplyr::rename(
                'Gene'            = GENE_SYMBOL,
                'DNA Change'      = varDNA, 
    ...
    {   
        ...
        VAR_LIST <- lapply( indexing(vidx), function(vx)
        {
            ...
            matchVARS <- var_pf %>% filter( 
                ...
                GENE_SYMBOL       %in% c11_preset_genes[[ PSG ]] 
            ) 
