...
#   update 2026-06-05
#   summary of change : Restored 'VARIANT_CLASS' to RequiredFields, OutputMafFields, and group_by to fix NULL insert in variants_summary DB table.
...
#---| MAF FIELDS |---------------------------------------------------------------------------------#
    # [MODIFIED] Added 'VARIANT_CLASS' back to RequiredFields
    RequiredFields <- c(
        "variant_call_mode","seq_folder","seq_id",'Hugo_Symbol', 'Chromosome', 'Start_Position', 'End_Position', "Strand","Reference_Allele", 
        "Tumor_Seq_Allele1", "Tumor_Seq_Allele2", "Match_Norm_Seq_Allele1", "Match_Norm_Seq_Allele2",
        'Variant_Classification', 'Variant_Type', 'VARIANT_CLASS', 'Tumor_Sample_Barcode','Matched_Norm_Sample_Barcode'
    )
    ...
    SelectFields <- c(SelectFields[1:73], "PURECN_FLAG", SelectFields[74:length(SelectFields)])
    
    RemoveClasses <- c("Silent","Intron","5'Flank","3'Flank","Targeted_Region","IGR","3'UTR","5'UTR","RNA")
    
    # [MODIFIED] Added 'VARIANT_CLASS' back to OutputMafFields to ensure it gets written to MAF
    OutputMafFields <- c(       
        "variant_call_mode","seq_folder","seq_id","Hugo_Symbol","Chromosome","Start_Position","End_Position","Strand","Reference_Allele","Tumor_Seq_Allele1","Tumor_Seq_Allele2",
        "Match_Norm_Seq_Allele1","Match_Norm_Seq_Allele2","Variant_Classification","Variant_Type","VARIANT_CLASS","Tumor_Sample_Barcode","Matched_Norm_Sample_Barcode",
...
#---| MAF FILE PREFIX & CALL MODE |----------------------------------------------------------------#
    if( is_gemlineMAF ) {
        MAF_TAG = "deepvariant.germline.variant.filtered"
        VAR_CALL_MODE = "Germline"
    } else {
        if( variantCallRunMode %in% c("tonly","Tonly") ) {
            ...
            VAR_CALL_MODE = ifelse( NORMAL_TYPE == "ORG", "ORG_N", "TS_N")
        }
    }
#--------------------------------------------------------------------------------------------------#
...
#---| MERGE MAF |----------------------------------------------------------------------------------#
    ...
    refseq_maf <- read.delim(refseq_file, skip=1, header=T) %>% 
        mutate(variant_call_mode = VAR_CALL_MODE, seq_folder = SEQ_FOLDER, seq_id = SEQ_ID)
...
#---| VARIANT CLASS SUMMARY |----------------------------------------------------------------------#
    ...
    # [MODIFIED] Added VARIANT_CLASS back to group_by
    VariantClassSummary <- rbind(
        mf_nonsyn %>% 
            group_by(Variant_Classification, Variant_Type, VARIANT_CLASS) %>% 
            summarise( stats=n(), .groups = 'drop' ) %>%
            mutate( variant_call_mode=VAR_CALL_MODE, seq_folder=SEQ_FOLDER, seq_id=SEQ_ID, variant_group='non_synonymous' ),
        mf_syn %>% 
            group_by(Variant_Classification, Variant_Type, VARIANT_CLASS) %>% 
            summarise( stats=n(), .groups = 'drop' ) %>%
            mutate( variant_call_mode=VAR_CALL_MODE, seq_folder=SEQ_FOLDER, seq_id=SEQ_ID, variant_group='synonymous' )
    )
#--------------------------------------------------------------------------------------------------#
    ...
    if( writeSomaticVarinatsOnlyMAF ) {
        ...
        if( is_gemlineMAF ) {
            ...
            VCS <- mf_nonsyn_filtered %>% 
                group_by(Variant_Classification, Variant_Type, VARIANT_CLASS) %>% 
                summarise( stats=n(), .groups = 'drop' ) %>%
                mutate( variant_call_mode=VAR_CALL_MODE, seq_folder=SEQ_FOLDER, seq_id=SEQ_ID, variant_group='filtered_somatic_only' )
...
if( writeDB ) {
    ...
    dbExecute(dbCon, sprintf("DELETE FROM variants_summary WHERE seq_folder = '%s' AND seq_id = '%s' AND variant_call_mode = '%s'", SEQ_FOLDER, SEQ_ID, VAR_CALL_MODE))
    
    # 1. Summary 적재
    summary_raw <- VariantClassSummary
    SafeMapAndInsert(dbCon, "variants_summary", summary_raw)

    # 2. Somatic Variants 적재
    if( writeSomaticVarinatsOnlyMAF ) {
        dbExecute(dbCon, sprintf("DELETE FROM variants_somatic WHERE seq_folder = '%s' AND seq_id = '%s' AND variant_call_mode = '%s'", SEQ_FOLDER, SEQ_ID, VAR_CALL_MODE))
        
        somatic_raw <- if(is_gemlineMAF) mf_nonsyn else mf_nonsyn_filtered
        
        inserted_rows <- SafeMapAndInsert(dbCon, "variants_somatic", somatic_raw)
        
        if(inserted_rows > 0) {
            write(sprintf("%s | %d filtered somatic variants IMPORTED INTO DB", format(now() ,format = "%Y-%m-%d %H:%M:%S"), inserted_rows), LOG_FILE, append=T)
        } else {
            write(sprintf("%s | [WARNING] Column mismatch or 0 rows. DB insert skipped.", format(now() ,format = "%Y-%m-%d %H:%M:%S")), LOG_FILE, append=T)
            message("[WARNING] Data could not be mapped to DB schema. Insert skipped.")
        }
    }
