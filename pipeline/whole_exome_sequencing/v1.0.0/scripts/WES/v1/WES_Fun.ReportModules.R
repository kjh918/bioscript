...
#---/ wes alignment stats barplot /-----------------------------------------------------------------
    WES_analysis.Alignment.Stats.Barplot <- function( SeqFolderId=SeqFolderID, SampleInfo=sinfo )
    {
        suppressPackageStartupMessages(library("plyr"))
        suppressPackageStartupMessages(library("dplyr"))
        suppressPackageStartupMessages(library("Hmisc"))
...
#---/ wes target coverage plot /--------------------------------------------------------------------
    WES_analysis.Target.Coverage.Plot <- function( SeqFolderId=SeqFolderID, SampleInfo=sinfo, X_LIMIT=NULL )
    {
        suppressPackageStartupMessages(library("plyr"))
        suppressPackageStartupMessages(library("dplyr"))
        suppressPackageStartupMessages(library("Hmisc"))
    ...
    WES_analysis.PresetGeneVariantsTable <- function( VariantData, CancerCode, SampleInfo, VariantCallMethod, PresetGeneList )
    {
        ...
        if( nrow(var_data) > 0 )
        {
            preset_var_res <- var_data %>% mutate(
                ...
                varDNA       = paste(REFSEQ_TRANSCRIPT_ID,HGVSc, sep=":")
            ) 
        }else{
...
#---/ preset gene variant table (report) /----------------------------------------------------------
    WES_report.PresetGeneVariantsTable <- function( PresetGeneVarTable )
    {
        ...
        TableOutput <- PresetGeneVarTable %>% dplyr::select(
                c("sample_name","GENE_SYMBOL", "varDNA","HGVSp_Short","DEPTH","TUMOR_ALT_COUNT","VAF")
            ) %>% dplyr::rename(
                'SAMPLE ID'=sample_name, 'GENE'= GENE_SYMBOL, 'DNA CHANGE'=varDNA, 'PROTEIN CHANGE'=HGVSp_Short, 'TUMOR ALT COUNT'=TUMOR_ALT_COUNT
...
#---/ preset gene variant heatmap /-----------------------------------------------------------------    
    WES_analysis.Preset.Gene.Variants.Heatmap <- function( PresetVariantsTable, CancerSampleInfo )
    {
        ...
        #-----------------------------------------------------------------------
        ## MOP
        var_mat <- PresetVariantsTable %>% group_by( GENE_SYMBOL, sample_name ) %>%
        ...
        #-----------------------------------------------------------------------
        VarMat <- reshape(var_mat , idvar = "GENE_SYMBOL", timevar = "sample_name", direction = "wide")
        ...
        if( length(MissIDs) > 0 )
        {
            ...
            VarMat <- VarMat[, c("GENE_SYMBOL", CancerSampleInfo$sample_name)]
        }
        #-----------------------------------------------------------------------
        rownames(VarMat) <- VarMat$GENE_SYMBOL
        #-----------------------------------------------------------------------
...
#---/ oncoplot target gene /------------------------------------------------------------------------
    ...
    )
    {
        ...
        opTargetGenes <- VariantData %>% filter( seq_id %in% SelectSeqID ) %>% 
            group_by( GENE_SYMBOL ) %>% reframe( varCount=length(GENE_SYMBOL) ) %>% arrange( desc(varCount) )
        # gene set filter ------------------------------------------------------            
        if( is.null(geneSets) )
        {
            if( is.null(topGenes) )
            {
                oncoPlotGenes <- opTargetGenes %>% .$GENE_SYMBOL %>% table() %>% sort() %>% rev()   
            }else{
                topGeneList   <- opTargetGenes %>% filter( varCount >= opTargetGenes$varCount[topGenes]) %>% .$GENE_SYMBOL
                if( length(topGeneList) > 30 )
                { 
                    topGeneList   <- opTargetGenes %>% filter( varCount >= opTargetGenes$varCount[topGenes] + 1 ) %>% .$GENE_SYMBOL
                } 
                oncoPlotGenes <- opTargetGenes %>% filter( GENE_SYMBOL %in% topGeneList ) %>% .$GENE_SYMBOL %>% table() %>% sort() %>% rev()
            }
        }else{
            geneSetGenes <- opTargetGenes %>% filter( GENE_SYMBOL %in% geneSets ) 
            if( useBoth )
            {
                if( nrow(geneSetGenes) >= topGenes )
                {
                    topGeneList   <- geneSetGenes %>% filter( varCount >= geneSetGenes$varCount[topGenes]) %>% .$GENE_SYMBOL
                    oncoPlotGenes <- geneSetGenes %>% filter( GENE_SYMBOL %in% topGeneList ) %>% .$GENE_SYMBOL %>% table() %>% sort() %>% rev()
                }else{
                    oncoPlotGenes <- geneSetGenes %>% .$GENE_SYMBOL %>% table() %>% sort() %>% rev()
                }
            }else{
                oncoPlotGenes <- geneSetGenes %>% .$GENE_SYMBOL %>% table() %>% sort() %>% rev()
            }
...
#---/ draw oncoplot /-------------------------------------------------------------------------------
    ...
    )
    {
        ...
        if( includeAllSamples )
        {
            ...
            {
                targetGeneCountTableExtra <- data.frame( seq_id = noVarSeqIDs, GENE_SYMBOL = NA, mutations = NA)
        ...
        oncoPlotMatrix <- oncoPlotMatrix[which(!is.na(oncoPlotMatrix$GENE_SYMBOL)), ]
        ...
        annotC <- HeatmapAnnotation(
            ...
            col    = list( TISSUE = tissueColors, GROUP = groupColors, ORIGIN = c("TS" = "#1F78B4", "FFPE" = "#838ee5", "TORG" = "#A6CEE3", "ORG" = "#A6CEE3", "BLOOD"="#F8CECC") ),
            gap    = unit(1.2, "mm"),
...
#---/ cumulative oncoplot data load from database /-------------------------------------------------
    WES_analysis.Get.Cumulative.Oncoplot.Data <- function( CancerCode, ClientID, VariantCallMethod, SampleLimit=10 )    
    {
        ...
        if( length(oncoplot2_target_samples) < SampleLimit )
        {
            ...
            dbCon <- dbConnect(dbDriver("MySQL"), host=db_host, user=db_user, port=db_port, password=db_pw, db=db_name_data )
            var_data <- dbGetQuery(dbCon, sprintf("SELECT seq_id,GENE_SYMBOL,Variant_Classification FROM variants_somatic WHERE variant_call_mode = '%s' AND seq_id IN (%s)", VariantCallMethod, querySeqID))
            dbDisconnect(dbCon)
...
#---/ individual matching heatmap /-----------------------------------------------------------------
    WES_analysis.Indiv.Match.Heatmap <- function( IndivMatchResult, SampleInfo )
    {
        ...
        #-----------------------------------------------------------------------
        im_sinfo <- as.data.frame(rbind(
            SampleInfo %>% filter( sample_group != "" ) %>% arrange( sample_info, sample_group, seq_id ),
            SampleInfo %>% filter( sample_group == "" ) %>% mutate( sample_group = "No Grouped" ) %>% arrange( sample_info, seq_id )
        ))
        #-----------------------------------------------------------------------
