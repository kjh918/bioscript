...
#---| GENERATE INDIV-MATCH-SNP-VCF |---------------------------------------------------------------#
    ...
    #     ${RUN_SAMTOOLS} mpileup -I -uf ${GENOME_FASTA} -l ${BED} ${INPUT_BAM_FILE} | /storage/apps/bcftools-1.18/bin/bcftools call -c - > ${IND_MATCH_VCF_DATA_DIR}/${SEQ_ID}.indiv.match.vcf
    #     echo -e " >> Generate Individual-Matching-SNP VCF: ${SEQ_ID} ....DONE!"
    # done

    parallel -j 15 -k "${RUN_SAMTOOLS} mpileup -I -uf ${GENOME_FASTA} -l ${BED} ${BASE_DIR}/${SEQ_FOLDER}/{}/bam/{}.analysisReady.bam | /storage/apps/bcftools-1.18/bin/bcftools call -c - > ${IND_MATCH_VCF_DATA_DIR}/{}.indiv.match.vcf" ::: $(cat ${IND_MATCH_RES_DIR}/indiv_match.list)
