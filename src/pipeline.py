...
class Task:
	def __init__(self, name: str, runner_path: Path, spec: Dict[str, Any], log_path: Path):
		...
		# 확장자에 따른 실행 타입 결정
		self.exec_type = "python3" if self.runner_path.suffix == ".py" else "bash"
	...
	def get_cmd(self) -> str:
		...
		return f"{self.exec_type} {shlex.quote(str(self.runner_path))} {' '.join(args)}"
		

class Pipeline:
	def __init__(self, raw_dir: str, work_dir: Path, log_dir: Path, executor: Any):
	...
	def _discover_samples(self) -> List[str]:
		...
		return sorted(list(set([f.name.split('_R1')[0] for f in self.raw_dir.glob("*_R1.fastq.gz")])))
